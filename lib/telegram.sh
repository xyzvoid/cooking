...
_TG_API="https://api.telegram.org/bot${TG_BOT_TOKEN:-__unset__}"
...
# ── Send a new text message, return message_id ───────────────────────────────
tg_send_message() {
...
# ── Send or edit a live-status message (edits in-place after first call) ─────
tg_send_or_edit() {
    _tg_enabled || return 0
    local text="$1"

    if [[ -z "$_TG_MSG_ID" ]]; then
        _TG_MSG_ID=$(tg_send_message "$text")
    else
        curl -fsSL -X POST \
